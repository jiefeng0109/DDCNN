# -*- coding: utf-8 -*-
"""
Created on Thu Jan 18 10:36:08 2018
...
new_bianjie_tran_loc = sio.loadmat(path + '/location/'+data_name+'/'+'bianjie_noe.mat')['new_bianjie_tran_loc'] 
loc_test_bianjie = sio.loadmat(path + '/location/'+data_name+'/'+'bianjie_noe.mat')['loc_test_bianjie'] 
new_bianjie_tran_label = sio.loadmat(path + '/location/'+data_name+'/'+'bianjie_noe.mat')['new_bianjie_tran_label'][0] 
...
X_train1 = get_input(w[2],new_bianjie_tran_loc,dim_input[0])
#X_train2 = get_input(w[2],train_loc,dim_input[2])
#X_train3 = get_input(w[3],train_loc,dim_input[3])
#X_test0 = get_input(w[0],loc_test_bianjie,dim_input[0])
X_test1 = get_input(w[2],loc_test_bianjie,dim_input[0])
#X_test2 = get_input(w[2],loc_test_bianjie,dim_input[2])
#X_test3_yunzhi = get_input(w[3],loc_test_yunzhi,dim_input[3])
...
Y_train = one_hot(new_bianjie_tran_label,dim_out )
...
index_train = np.arange(X_train1.shape[0])
np.random.shuffle(index_train)
#train_loc =train_loc[:,index_train]
#X_train0 = X_train0[index_train, :]
X_train1 = X_train1[index_train, :]
#X_train2 = X_train2[index_train, :]
#X_train3 = X_train3[index_train, :]
#X_train4 = X_train4[index_train, :]
...
x1 = tf.placeholder(tf.float32,[None,w[2],w[2],dim_input[0]],name='x_input1')
...
weights={'W10':tf.Variable(tf.truncated_normal([1,1,dim_input[0],32],stddev=0.1)),
         
         'W11':tf.Variable(tf.truncated_normal([1,1,dim_input[1],32],stddev=0.1)),

         'W12':tf.Variable(tf.truncated_normal([1,1,dim_input[2],32],stddev=0.1)),
         
         'W13':tf.Variable(tf.truncated_normal([3,3,dim_input[3],32],stddev=0.1)),
         
         'W2':tf.Variable(tf.truncated_normal([1,1,32,64],stddev=0.1)),
         
         'W23':tf.Variable(tf.truncated_normal([3,3,32,64],stddev=0.1)),        

         'W3':tf.Variable(tf.truncated_normal([1,1,64,128],stddev=0.1)),

         'W33':tf.Variable(tf.truncated_normal([3,3,64,128],stddev=0.1)), 

         'W4':tf.Variable(tf.truncated_normal([1,1,128,256],stddev=0.1)),
         
         'W6':tf.Variable(tf.truncated_normal([1,1,256,256],stddev=0.1)),

         'W43':tf.Variable(tf.truncated_normal([3,3,128,256],stddev=0.1)),
 
         'W5':tf.Variable(tf.truncated_normal([256,dim_out],stddev=0.1)),
                                              
         'W53':tf.Variable(tf.truncated_normal([256*4,dim_out],stddev=0.1)),
        }

bias={'B1':tf.Variable(tf.constant(0.1,shape=[32])),
...
def branch1(x1,weights,bias,dropout,w,dim_input):
    ...
    conv1 = conv2dlayer(x1,weights['W10'],bias['B1'],[1,1,1,1])
    pool1 = tf.nn.max_pool(conv1,[1,2,2,1],[1,2,2,1],padding='SAME', name='POOL1')
    conv2=conv2dlayer(pool1,weights['W2'],bias['B2'],[1,1,1,1])
    dpt1 = tf.nn.dropout(conv2,dropout,name='dropout')
    pool2 = tf.nn.max_pool(dpt1,[1,2,2,1],[1,2,2,1],padding='SAME', name='POOL2')
...
def branch2(x2,weights,bias,dropout,w,dim_input):
    ...
    conv1 = conv2dlayer(x2,weights['W10'],bias['B1'],[1,1,1,1])
    pool1 = tf.nn.max_pool(conv1,[1,2,2,1],[1,2,2,1],padding='SAME', name='POOL1')
    conv2=conv2dlayer(pool1,weights['W2'],bias['B2'],[1,1,1,1])
    dpt1 = tf.nn.dropout(conv2,dropout,name='dropout')
    pool2 = tf.nn.max_pool(dpt1,[1,2,2,1],[1,2,2,1],padding='SAME', name='POOL2')
...
#    conv5=conv2dlayer(pool4,weights['W6'],bias['B4'],[1,1,1,1])
    reshape = tf.reshape(pool4,[-1,weights['W5'].get_shape().as_list()[0]])
    return reshape
def branch3(x3,weights,bias,dropout,w,dim_input):
    x3 = tf.reshape(x3,shape=[-1,w,w,dim_input])
    conv1 = conv2dlayer(x3,weights['W13'],bias['B1'],[1,1,1,1])
    pool1 = tf.nn.max_pool(conv1,[1,2,2,1],[1,2,2,1],padding='SAME', name='POOL1')
    ...
    pool4=tf.nn.max_pool(conv4,[1,2,2,1],[1,2,2,1], padding='SAME', name='POOL4')
    reshape = tf.reshape(pool4,[-1,weights['W5'].get_shape().as_list()[3]*4])
    return reshape
...
def get_oa(X_valid):
    ...
    while index_all<num:
        ...
        index_all += step
        temp1 = y_.eval(feed_dict={x1: input,dropout:1})
...
reshape1 = branch2(x1,weights,bias,dropout,w[2],dim_input[0])  
#reshape2 = branch2(x2,weights,bias,dropout[0],w[2],dim_input[2])
#reshape3 = branch3(x3,weights,bias,dropout[0],w[3],dim_input[3])
#reshape = tf.concat([reshape0,reshape1,reshape2],1)
#dpt = tf.nn.dropout(reshape,dropout[2])
#gate = gate(x4,w[2],dim_input[4],threshold)
#f1 = reshape*gate
f2 = tf.add(tf.matmul(reshape1,weights['W5']),bias['B5'])
...
#    batch_x4 = next_batchx(X_train4,index)
        ...
        sess.run(train_step, feed_dict={x1: batch_x1,y: batch_y,dropout:0.5})
        if step%display_step == 0:
            loss,acc = sess.run([cross_entropy,accuracy], feed_dict={x1: batch_x1, y: batch_y,dropout:0.5})
            print('step %d,training accuracy %f,cross_entrop %f'%(step,acc,loss))
        #print("step" + str(step) + ", Minibatch Loss= " + \
                  #"{:.6f}".format(loss) + ", Training Accuracy= " + \
                  #"{:.5f}".format(acc))
            print("test accuracy %g"%accuracy.eval(feed_dict={x1: X_test1,y: Y_test_bianjie,dropout:1}))
        index = index+batch_size
        step += 1
        if index>X_train1.shape[0]:
            index = batch_size
            epoch=epoch+1
...
#    y_pred = get_oa(X_test1)
#    y_tr=contrary_one_hot(Y_test_bianjie).astype('int32')
    temp = y_.eval(feed_dict={x1: X_test1,y: Y_test_bianjie,dropout:1})
    ...
    sio.savemat(path+'/label/'+data_name+'/'+'label_z.mat',{'label_bianjie_tr':y_tr,'label_bianjie_pre':y_pred})
'''
#    data_all_dir = path + '/data_all/' + data_name
#    data = sio.loadmat(data_all_dir + '/' + data_name + 'bj_yz.mat')
#    data_all = data['bianjie_all']
    loc = sio.loadmat(path+'/label/'+data_name+'/'+'loc.mat')
    loc_all = loc['loc_all_bianjie'].astype('int32')
    data_all = get_input(w[2],loc_all,dim_input[0])
    data_all,loc_all =  shuffdata(data_all,loc_all)
    y_pred = get_oa(data_all)
#    temp1=y_.eval(feed_dict={x1: data_all,dropout:1})
#    y_pred=contrary_one_hot(temp1).astype('int32')
    sio.savemat(path+'/label/'+data_name+'/'+'label_all_bianjie.mat',{'label__all_bianjie_pre':y_pred})
    sio.savemat(path+'/label/'+data_name+'/'+'locx.mat',{'loc_all_bianjie':loc_all})
'''
...
with tf.Session() as sess1:
    ...
    while epoch<num_epoch:
        ...
        sess1.run(train_step, feed_dict={x1: batch_x1,y: batch_y,dropout:0.5})
        index = index+batch_size
        step += 1
        if index>X_train1.shape[0]:
            index = batch_size
...
#    y_tr=contrary_one_hot(Y_test_bianjie).astype('int32')
    ...
    sio.savemat(path+'/label/'+data_name+'/'+'label1_z.mat',{'label_bianjie_tr':y_tr,'label_bianjie_pre':y_pred})
...
with tf.Session() as sess2:
    ...
    while epoch<num_epoch:
        ...
        sess2.run(train_step, feed_dict={x1: batch_x1,y: batch_y,dropout:0.5})
        index = index+batch_size
        step += 1
        if index>X_train1.shape[0]:
            index = batch_size
...
#    y_tr=contrary_one_hot(Y_test_bianjie).astype('int32')
    ...
    sio.savemat(path+'/label/'+data_name+'/'+'label2_z.mat',{'label_bianjie_tr':y_tr,'label_bianjie_pre':y_pred})
...
with tf.Session() as sess3:
    ...
    while epoch<num_epoch:
        ...
        sess3.run(train_step, feed_dict={x1: batch_x1,y: batch_y,dropout:0.5})
        index = index+batch_size
        step += 1
        if index>X_train1.shape[0]:
            index = batch_size
...
#    y_tr=contrary_one_hot(Y_test_bianjie).astype('int32')
    ...
    sio.savemat(path+'/label/'+data_name+'/'+'label3_z.mat',{'label_bianjie_tr':y_tr,'label_bianjie_pre':y_pred})
...
with tf.Session() as sess4:
    ...
    while epoch<num_epoch:
        ...
        sess4.run(train_step, feed_dict={x1: batch_x1,y: batch_y,dropout:0.5})
        index = index+batch_size
        step += 1
        if index>X_train1.shape[0]:
            index = batch_size
...
#    y_tr=contrary_one_hot(Y_test_bianjie).astype('int32')
    ...
    sio.savemat(path+'/label/'+data_name+'/'+'label4_z.mat',{'label_bianjie_tr':y_tr,'label_bianjie_pre':y_pred})
