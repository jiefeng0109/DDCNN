...
train_size = np.shape(train_loc)
test_size = np.shape(test_loc)
loc_train_bianjiex = []
loc_train_yunzhix = []
loc_train_bianjiey = []
loc_train_yunzhiy = []
...
for i in range(dim_out):
    ...
    for j in range(num_ev):
        ith = start+j
#        ratio = neibor_ratio(train_loc[0][ith],train_loc[1][ith],w[2],mean,mingaus)
        most_label,ratio1 = neibor_vote1(train_loc[0][ith],train_loc[1][ith],w[2])
#        if (ratio<0.95)and(ratio1<0.95):
        if (ratio1<=0.9):
            loc_train_bianjiex.append(train_loc[0][ith])
            loc_train_bianjiey.append(train_loc[1][ith])
            label_train_bianjie.append(y_train[ith])
            countb+=1
        else :
            loc_train_yunzhix.append(train_loc[0][ith])
            loc_train_yunzhiy.append(train_loc[1][ith])
            label_train_yunzhi.append(y_train[ith])
            county+=1    
    print(countb,county)
    countb = 0
    county = 0
loc_train_bianjie = np.vstack([loc_train_bianjiex,loc_train_bianjiey])        
loc_train_yunzhi = np.vstack([loc_train_yunzhix,loc_train_yunzhiy])
print('train bianjie shape',loc_train_bianjie.shape)
print('train yunzhi shape',loc_train_yunzhi.shape)
'''
expand_loc = sio.loadmat(path + '/expand/'+data_name+'/expand_500.mat')['expand_loc']
expand_label = sio.loadmat(path + '/expand/'+data_name+'/expand_500.mat')['expand_label'][0]
cum_exe_list = sio.loadmat(path + '/expand/'+data_name+'/expand_500.mat')['cum_exe_list'][0]
num_exe = sio.loadmat(path + '/expand/'+data_name+'/expand_500.mat')['num_exe'][0]
loc_exe_bianjiex = []
loc_exe_yunzhix = []
loc_exe_bianjiey = []
loc_exe_yunzhiy = []
...
#    ratio = neibor_ratio(test_loc[0][i],test_loc[1][i],w[2],mean,mingaus)
    most_label,ratio1 = neibor_vote1(test_loc[0][i],test_loc[1][i],w[2])
#    if (ratio<0.95)and(ratio1<0.95):
    if (ratio1<=0.9):
...
#    if ((ith_label+1)!=y_test[i]):
#        print(i,ith_label,y_test[i])
        count+=1
    if i%10000==0:
        print(i)
print(count)
...
sio.savemat(path + '/location/'+data_name+'/'+'/yunzhi_noe.mat',{'new_yunzhi_tran_loc':new_yunzhi_tran_loc,'new_yunzhi_tran_label':new_yunzhi_tran_label,'loc_test_yunzhi':loc_test_yunzhi,'label_test_yunzhi':label_test_yunzhi})
sio.savemat(path + '/location/'+data_name+'/'+'/bianjie_noe.mat',{'new_bianjie_tran_loc':new_bianjie_tran_loc,'new_bianjie_tran_label':new_bianjie_tran_label,'loc_test_bianjie':loc_test_bianjie,'label_test_bianjie':label_test_bianjie})
#loc_test_bianjiex = []
#loc_test_yunzhix = []
#loc_test_bianjiey = []
#loc_test_yunzhiy = []
